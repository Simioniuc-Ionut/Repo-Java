...
public class TournamentScheduler {
    ...
    private void printSchedule(List<List<Integer>> scheduleDays) {
        for (int dayIndex = 0; dayIndex < scheduleDays.size(); dayIndex++) {
            ...
            for (int i = 0; i < daySchedule.size(); i += 2) {
                ...
                System.out.println("match between player " + (player1 + 1) + " and player " + (player2 + 1));
                System.out.println("winner: " + (matchResults.get(player1 + "-" + player2) + 1));
            }
        }
    }

    private void findWinningSequence() {
        ...
        for (int i = 0; i < n; i++) {
            if (!visited[i]) {
                if (findSequenceUtil(i, victoryGraph, visited, sequence)) {
                    break;
                }
            }
        }
    ...
    private boolean findSequenceUtil(int current, Map<Integer, Set<Integer>> graph, boolean[] visited, List<Integer> sequence) {
        ...
        for (int neighbor : graph.get(current)) {
            ...
            if (allNeighborsDefeated) {
                if (findSequenceUtil(neighbor, graph, visited, sequence)) {
                    return true;
                }
            }
        }

        // Daca ajungem aici, inseamna ca nu am putut gasi o secventa valida pornind de la nodul curent
    ...
    public static void main(String[] args) {
        TournamentScheduler scheduler = new TournamentScheduler(3, 2, 2);
